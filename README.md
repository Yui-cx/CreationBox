...
CreationBox 是一个面向文本编写创作的 AI 创作工作台 MVP。支持 AI 痕迹去除、降 AIGC 率、文章生成、图片水印去除等智能创作工具。提供历史记录、文件导出、管理员用户管理和模型配置等基础管理能力。

## 功能点

- 智能创作：支持 AI 痕迹去除、降 AIGC 率、文章生成等文本工具。
- 水印去除：提供基于 OpenCV 和 LaMa 的两种图像修复方案。
- 历史记录：保存生成任务，可查看、加载、删除历史内容。
- 文件导出：生成结果可导出为 Markdown 文件，便于二次编辑。
